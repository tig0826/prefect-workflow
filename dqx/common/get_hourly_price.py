    ...
    for forcus_item_list in focus_item_matrix:
        ...
        for item_name in forcus_item_list:
            ...
            if item_name in ["輝晶核", "閃輝晶核"]:
                ...
                five_percentile_price = df_item[df["個数"] <= quantity_threshold)]["1つあたりの個数"].quantile(percentile)
            else:
                five_percentile_price = df_item["1つあたりの価格"].quantile(percentile)
            message_price = f"**{item_name}** : " + "{:,}".format(int(five_percentile_price))+"G\n"
            message += message_price
        # 利益の期待値を計算
        # 細胞1個かかけら20個の内の安い方で計算
        price_saibou_base = min(df[df["Name"] == f"{saibou_name}"]["1つあたりの価格"].quantile(percentile),
                               df[df["Name"] == f"{kakera_name}"]["1つあたりの価格"].quantile(percentile)*20)
        # Nameが輝晶核で列名"個数"の値がquantity_threshold以上の核のデータを排除
        price_kaku_base = df[
                (df["Name"] == f"{kaku_name}") & (df["個数"] <= quantity_threshold)
