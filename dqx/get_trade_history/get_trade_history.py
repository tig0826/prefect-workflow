        ...
        for row in rows[1:]:
            ...
            if trade_day == yesterday:
                exhibition_data.append([item_name,
                                        ...
                                        trade_partner])
        pagenum += 1
    df = pd.DataFrame(exhibition_data, columns=["アイテム名", "種類", "できのよさ", "個数", "価格", "Date", "取引相手"])
    return df
...
def get_trade_history():
    ...
    schema_name = "dqx"
    # 購入情報を取得
    table_name = "history_buy"
    df_buy = get_trade_buy(session)
    if len(df_buy) > 0:
        trino.create_table(table_name,
                           schema_name,
                           trino.extract_columns(df_buy),
                           partitioning=["Date"])
        trino.insert_table(table_name, schema_name, df_buy)
    # 売却情報を取得
    table_name = "history_sell"
    df_sell = get_trade_sell(session)
    if len(df_sell) > 0:
        trino.create_table(table_name,
                           schema_name,
                           trino.extract_columns(df_sell),
                           partitioning=["Date"])
        trino.insert_table(table_name, schema_name, df_sell)
