...
WITH src AS (
    ...
    {% if is_incremental() %}
      AND s.stay_start_time_jst >= CAST(date_add(
        'day',
        -{{ reprocess_days }},
...
SELECT
    ...
    CAST(stay_start_time_jst AS date) AS event_date_jst,
    'owntracks' AS source_system,
    'owntracks_outing' AS source_detail,
    CAST(stay_start_time_jst AS TIMESTAMP) AS start_ts,
    CAST(stay_end_time_jst AS TIMESTAMP) AS end_ts,
